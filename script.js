function validateForm() {
  ...
  let employmentStatus =
    document.forms["githubGroup"]["employmentStatus"].value;
  let programmingLanguage = document.querySelectorAll(
    "input[name=programmingLanguage]:checked"
  );
  let timeZone = document.forms["githubGroup"]["timeZone"].value;
  let interestedActivities = document.querySelectorAll(
    "input[name=interestedActivities]:checked"
  );

   let regxName = /^([a-z\d]+-)*[a-z\d]+$/i;

  if (!regxName.test(uname.value)) {
    uname.style.border = "solid 2px red";
    document.getElementById("unameLabel").innerHTML =
      "The name can only have alphabetic characters.";
